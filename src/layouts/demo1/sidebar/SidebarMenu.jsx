...
const SidebarMenu = ({ menuItems = [] }) => {
  ...
  // Use passed menuItems if provided; otherwise, fall back to default menu config.
  const menuConfig = menuItems.length > 0 ? menuItems : getMenuConfig('primary');
  ...
  const buildMenuItemRoot = (item, index) => {
    if (item.children) {
      return (
        <MenuItem key={index} {...(item.toggle && { toggle: item.toggle })} {...(item.trigger && { trigger: item.trigger })}>
  ...
  const buildMenuItemChild = (item, index, level = 0) => {
    if (item.children) {
      return (
        <MenuItem key={index} {...(item.toggle && { toggle: item.toggle })} {...(item.trigger && { trigger: item.trigger })} className={clsx(item.collapse && 'flex-col-reverse')}>
